...
def init_params_add_wordvec(tokendict, vector_bin, n_chars=N_WORD):
    """
    Loads 300x1 word vecs from Google (Mikolov) word2vec
    """
    num_wordvec_found = 0
    params = init_params(N_WORD)
    word_vecs = np.random.normal(loc=0., scale=SCALE, size=(n_chars,WORD_DIM)).astype('float32')
    with open(vector_bin, "rb") as f:
        ...
        for line in xrange(vocab_size):
            ...
            while True:
                ...
                if ch != '\n':
                    word.append(ch)   
            if word in tokendict and tokendict[word] < n_chars:
                num_wordvec_found += 1
                word_vecs[tokendict[word]] = np.fromstring(f.read(binary_len), dtype='float32')  
            else:
                f.read(binary_len)
    params["Wc"].set_value(word_vecs)
    print "total word: %s, wordvev found: %s" % (n_chars, num_wordvec_found)
    return params
...
def tweet2vec(tweet,mask,params,n_chars=N_WORD):
    ...
    l_f_source_tmp = lasagne.layers.NINLayer(l_fgru_source, num_units=1, W=params['W_c2w_f_a'], b=None)
    l_b_source_tmp = lasagne.layers.NINLayer(l_bgru_source, num_units=1, W=params['W_c2w_b_a'], b=None)
    l_f_source = lasagne.layers.SliceLayer(l_f_source_tmp, 0, axis=1)
    l_b_source = lasagne.layers.SliceLayer(l_b_source_tmp, 0, axis=1)
